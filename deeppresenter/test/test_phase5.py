...
def _verdict(verdict="pass", **kwargs):
    value = {
        "verdict": verdict,
        "severity": 0 if verdict != "fail" else 1,
        "confidence": 0.9,
...
def _artifact(*, overflow=False, width=1280, height=720):
    ...
    computed = ComputedSlideElement(
        ...
        client_width=100,
        client_height=20,
        scroll_width=120 if overflow else 100,
        scroll_height=20,
    )
...
def test_critic_requires_explicit_multimodal_capability():
    config = {
        "file_path": "x",
        "research_agent": {"model": "m", "api_key": "x"},
        "design_agent": {"model": "m", "api_key": "x"},
        "long_context_model": {"model": "m", "api_key": "x"},
        "critic_model": {"model": "critic", "api_key": "x"},
    }
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_atomic_s1_payload_is_single_class_structured_and_stateless(
    tmp_path: Path,
):
    ...
    result = await TitleBodyMismatchInspector(client).inspect(
        InspectionContext(artifact=_artifact(), render_path=str(image))
    )
    ...
    assert (
        "exactly one defect class: S1" in prompt
        and "mutation" not in prompt.lower().replace("mutation metadata", "")
    )
...
async def test_invalid_json_and_missing_verdict_become_error():
    for raw in ("not json", '{"severity": 0, "confidence": 1, "evidence": ["x"]}'):
        result = await TitleBodyMismatchInspector(
            AtomicNeuralClient(FakeLLM([raw]))
        ).inspect(InspectionContext(artifact=_artifact()))
...
async def test_provider_without_image_support_is_capability_error(tmp_path: Path):
    ...
    result = await TitleBodyMismatchInspector(AtomicNeuralClient(model)).inspect(
        InspectionContext(artifact=_artifact(), render_path=str(image))
    )
    assert result.status == InspectionStatus.ERROR
...
async def test_s4_deterministic_extreme_avoids_model():
    ...
    model = FakeLLM([])
    result = await DensityInspector(AtomicNeuralClient(model)).inspect(
        InspectionContext(artifact=artifact)
    )
...
async def test_s6_missing_render_defers_and_g7_dom_fail_avoids_model():
    ...
    s6 = await ImageTextContradictionInspector(client, "image", "body").inspect(
        InspectionContext(artifact=_artifact())
    )
    g7 = await RenderAnomalyInspector(client, "body").inspect(
...
async def test_reference_missing_defers_and_order_control(tmp_path: Path):
    ...
    missing = await ReferenceInspector(AtomicNeuralClient(FakeLLM([]))).inspect(
        InspectionContext(artifact=_artifact()), DefectClass.S6, "contradiction"
    )
    ...
    result = await ReferenceInspector(AtomicNeuralClient(FakeLLM(responses))).inspect(
        InspectionContext(
            artifact=_artifact(),
            render_path=str(target_path),
            reference_artifact=_artifact(),
            reference_render_path=str(ref_path),
        ),
        DefectClass.S6,
        "contradiction",
    )
...
async def test_failure_attribution_reproduces_abc_and_budget(tmp_path: Path):
    ...
    responses = [
        _verdict("defer", defer_reason="not visible"),
        _verdict("fail"),
        _verdict("fail"),
        _verdict("fail"),
        _verdict("fail"),
    ...
    attribution = await FailureAttributor(AtomicNeuralClient(model)).run(
        ...
        {"title": "A", "body": "B"},
        whole_rubric_definition="whole rubric baseline",
        repeated_whole_rubric_budget=2,
    )
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_POST(self):
            ...
            if request_index < len(statuses_before_pass):
                ...
                self.end_headers()
                self.wfile.write(
                    b'{"error":{"message":"rate limited","type":"rate_limit_error","code":"rate_limit"}}'
                )
                return
...
def test_human_reviewed_atomic_prompt_fixtures_remain_single_defect():
    for defect_class, definition in ATOMIC_PROMPT_FIXTURES.items():
        prompt = AtomicNeuralClient._atomic_prompt(
            defect_class, definition, {"element_ids": ["target"]}
        )
...
async def test_live_atomic_model_from_dotenv():
    ...
    verdict, record = await AtomicNeuralClient(model).inspect(
        ...
        {
            "titles": [{"element_id": "title", "text": "Solar growth"}],
            "bodies": [{"element_id": "body", "text": "Solar capacity doubled."}],
        },
    )
    assert verdict.verdict == "pass"
    assert record.usage
